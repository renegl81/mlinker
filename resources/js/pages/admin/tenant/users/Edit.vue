...
const breadcrumbItems: BreadcrumbItem[] = [
    ...
    { title: messages.users.actions.edit, href: '#' },
];

const initialScope = props.user.tenant_permissions?.scope === 'locations' ? 'locations' : 'all';
const initialLocationIds = (props.user.tenant_permissions?.location_ids ?? []) as number[];
...
const form = useForm({
    ...
    role: (props.user.tenant_role ?? 'editor') as 'owner' | 'editor',
    location_scope: initialScope as 'all' | 'locations',
    location_ids: initialLocationIds,
});
...
const isSelfEdit = props.user.id === props.currentUserId;
</script>

<template>
    <Head :title="messages.users.actions.edit" />
    <AppLayout :breadcrumbs="breadcrumbItems">
        <div class="flex h-full flex-1 flex-col gap-5 rounded-xl p-4 max-w-3xl mx-auto w-full">
            <div class="flex items-center gap-3">
                ...
                <HeadingSmall
                    :title="messages.users.actions.edit"
